...
/* send cmd to child and block for reply */
static int hub_send_command_to_child(hub_device *hub,
    ...
    rc = request_reply_timeout(target_fifo, reply_fifo, &req, child_resp, TIMEOUT_DEVICE);
    if (rc != OK) {
        return rc;
    }

    if (child_resp->request_id != req.request_id) {
        return ERR_IPC_FAILURE;
    }

    if (child_resp->src_id != child_id) {
        return ERR_IPC_FAILURE;
    }

    if (child_resp->dst_id != hub->base.info.id) {
        return ERR_IPC_FAILURE;
    }
...
/* main message dispatcher for hub */
static int hub_handle_message(device *dev,const domo_message *req,domo_message *resp) {
    ...
    if(strcmp(req->command,CMD_SWITCH)==0) {
        ...
        if (rc != OK) {
            resp->status = rc;
            snprintf(resp->payload, sizeof(resp->payload),
                    "hub %d switch failed", dev->info.id);
            return OK;
        }

        resp->status = OK;
        snprintf(resp->payload, sizeof(resp->payload),
                "hub %d switched %s", dev->info.id, position);
        return OK;
    }
